...
jQuery(function ($) {
  // Buscamos el contenedor específico del stock dentro de la variación
  const $variationBox = $('.woocommerce-variation.single_variation');

  const observer = new MutationObserver(() => {
    // Buscamos la clase específica de disponibilidad que usa WooCommerce
    const $stockContainer = $variationBox.find('.woocommerce-variation-availability');
    const text = $stockContainer.text().trim();

    if (!text) return; // Si no hay texto de stock aún, salir

    // Buscamos el número que acompaña a la palabra "disponibles"
    // Esto evita capturar el precio
    const match = text.match(/(\d+)\s*disponibles/i);
